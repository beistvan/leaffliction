...
class ImageTransformer:
    ...
    def __init__(self, parameters):
        ...
        if self.parameters.writeimg:
            self.name_save = self.parameters.outdir + \
                "/" + extract_file_stem(self.parameters.image)
...
class Parameters:
    """
    Parameters for the transformation process.
    """
...
def run_image_transformation(params, training=False):
    """
    Generate image transformation for the given parameters.
...
def adjust_the_path(src, path):
    """
    Adjust the path to the image in the destination directory.
    """
    src = src.rstrip("/") + "/"
    last = extract_file_stem(src)
    relative_path = path[len(src):]
    if not relative_path:
        return last
    return f"{last}/{relative_path}"


def run_batch_image_transformation(src, dst, training=False):
    """
    Generate image transformation for each image found from the source
    directory. Training argument used when calling batch_transform_images
    before training a model.
    """
    if src is None or dst is None:
        raise ValueError(
            "Both 'src' (source directory) and 'dst' (destination directory) "
            "must be specified."
        )
    ...
    if not os.path.isdir(dst):
        os.makedirs(dst)
    print("Batch processing images from", src, "to", dst)
    print("Found", len(os.listdir(src)), "pictures")
    print("Processing images...")

    for root, _, files in os.walk(src):
        name = adjust_the_path(src, root)
        jpg_files = [file for file in files if file.endswith(".JPG")]
        for file in tqdm(jpg_files):
            dst_path = os.path.join(dst, name, file)
            if processing_is_finished(dst_path):
                continue
            params = Parameters(
            ...
            run_image_transformation(params, training)
    print()


def extract_file_stem(path):
    """
    Return the filename without its directory or extension.
    """
...
def processing_is_finished(path):
    """
...
def display_images(params):
    ...
    """
    filename = params.outdir + "/" + extract_file_stem(params.image)
...
def main():
    """
    Main function to handle command line arguments and call the
    ...
    parser.add_argument(
        "-src",
        type=str,
        help="Path to the source dir with images.")
    parser.add_argument(
        "-dst",
        type=str,
        help="Path to the destination dir or image.")
    parser.add_argument(
        "-t",
        "--training",
        action="store_true",
        help="Run in training mode (default: False).")
    parser.add_argument(
        "-k",
        "--keep",
        action="store_true",
        help="Keep the temporary output in ./images (default: delete)")
    args = parser.parse_args()

    if args.img and os.path.isfile(args.img):
        if not args.img.endswith(".JPG"):
            print("Sorry, the file provided is not a JPG image.")
            sys.exit(1)
        params = Parameters(args.img, outdir="./images")
        run_image_transformation(params)
        display_images(params)
        print(f"Images saved in {params.outdir}")
    elif args.src and args.dst:
        run_batch_image_transformation(args.src, args.dst, args.training)
        print(f"Batch processing completed. Results saved in {args.dst}")
    else:
        print("Please provide a valid image or directory path.")
        print("  For single image: python Transformation.py <img>")
        print("  For batch: python Transformation.py -src <src_dir> "
              "-dst <dst_dir> [--training]")
        sys.exit(1)
